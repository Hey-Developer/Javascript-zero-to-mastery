...
# Properties:
  ...
  -> Response.type (Read only) :The type of the response (e.g., basic, cors).
  ...
  -- Response implements Body, so it also has the following properties available to it:
     ->Body.body (Read only) : A simple getter exposing a ReadableStream of the body contents.
...
# Methods: 
  ...
  -- Body Interface Methods: Response implements Body, so it also has the following methods available to it:
